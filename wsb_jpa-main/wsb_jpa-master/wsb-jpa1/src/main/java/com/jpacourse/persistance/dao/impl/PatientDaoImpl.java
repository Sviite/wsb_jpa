...
public class PatientDaoImpl extends AbstractDao <PatientEntity,Long> implements PatientDao {
    ...
    public void addVisitToPatient(Long patientId, Long doctorId, LocalDateTime date, String description) {
        ...
        patient.getVisitEntityCollection().add(visitEntity);
        doctor.getVisitEntityCollection().add(visitEntity);
    ...
    public List<PatientEntity> findByMoreVisits(long visNumber){
        return entityManager.createQuery("select pat from PatientEntity pat" + " join pat.visitEntityCollection vis where size(vis)>:param1" ,PatientEntity.class).setParameter("param1",visNumber).getResultList();
    }
